...
app.get('/', function(req, res) {
    ...
    renderResult(null, layers, tints, 0, res);
...
var renderResult = function(masterImage, layers, tints, layerIndex, res) {
        ...
        response.on('end', function() {
            ...
            lwip.open(new Buffer(imagedata, 'binary'), 'png', function(err, image) {
                ...
                function collapseImage() {
                    if (masterImage) {
                        masterImage.paste(0, 0, image, function(err, image) {
                            if (err) {
                                res.send(err);
                            }
                            masterImage = image;

                            renderResult(masterImage, layers, tints, ++layerIndex, res);
